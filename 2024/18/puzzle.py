import time
from heapq import heappop, heappush
from collections import defaultdict
from math import inf


class Puzzle:

    def __init__(self, data: str):
        self.data = data
        self._grid = None
        self.start = (0, 0)
        self.end = (70, 70)

    @property
    def answer(self) -> int:
        return 0

    def _parse(self) -> tuple[tuple[int, int]]:
        _grid = []
        for coords in self.data.split('\n'):
            _grid.append(tuple([int(a) for a in coords.split(',')]))
        return tuple(_grid)

    @property
    def grid(self) -> str:
        if self._grid is None:
            self._grid = self._parse()
        output = ''
        for column in range(self.end[0] + 1):
            for row in range(self.end[1] + 1):
                output += self.get_char((row, column))
            output += '\n'
        return output

    def get_char(self, coords: tuple[int, int]) -> str:
        if coords in self._grid:
            return '#'
        if coords == self.start:
            return 'S'
        if coords == self.end:
            return 'E'
        return '.'


class Maze:

    def __init__(self, data, pause=0):
        self.history = defaultdict(set)
        self.pause = pause
        self.visited = []
        self.grid = {}
        for y, row in enumerate(data.split("\n")):
            for x, char in enumerate(row):
                self.grid[(x, y)] = char
        self.start = ""
        self.end = ""
        for k, v in self.grid.items():
            if v == "S":
                print("Start", k)
                self.start = k
            elif v == "E":
                print("End", k)
                self.end = k
        self.max_x = x
        self.max_y = y
        if self.pause:
            print(self.start, self.end)
            print(self.show_grid())

    def get_neighbor_positions(
        self, pos: tuple[int, int], direction: tuple[int, int]
    ) -> list[tuple[tuple[int, int], tuple[int, int], int]]:
        return [
            ((pos[0] + direction[0], pos[1] + direction[1]), direction, 1),
            (
                (pos[0] + (direction[1] * -1), pos[1] + (direction[0] * -1)),
                (direction[1] * -1, direction[0] * -1),
                1,
            ),
            (
                (pos[0] + direction[1], pos[1] + direction[0]),
                (direction[1], direction[0]),
                1,
            ),
        ]

    def get_neighbors(self, pos: tuple[int, int], direction: tuple[int, int]):
        neighbors = []
        neighbor_positions = self.get_neighbor_positions(pos, direction)
        for new_pos, new_dir, score in neighbor_positions:
            if self.is_valid(new_pos[0], new_pos[1]):
                neighbors.append((score, (new_pos, new_dir)))
        return neighbors

    def is_valid(self, x: int, y: int):
        return (
            0 <= x < self.max_y + 1
            and 0 <= y < self.max_x + 1
            and self.grid[(x, y)] in (".", "E")
        )

    def dijkstra(self):
        start = (self.start, (1, 0))
        queue = [(0, start)]
        dists = {start: 0}

        while len(queue) > 0:
            if self.pause > 0:
                print(self.show_grid())
            dist, (position, direction) = heappop(queue)

            if position == self.end:
                return (
                    dist,
                    direction,
                    len(self.get_solution_path((self.end, direction))),
                )

            for cost, (npos, ndir) in self.get_neighbors(position, direction):
                dist_cost = dist + cost

                if dist_cost < dists.get((npos, ndir), inf):
                    dists[(npos, ndir)] = dist_cost
                    self.history[(npos, ndir)] = {(position, direction)}
                    if self.pause:
                        self.visited = [k for (k, v) in dists.keys()]
                    heappush(queue, (dist_cost, (npos, ndir)))
                elif dist_cost == dists.get((npos, ndir), inf):
                    dists[(npos, ndir)] = dist_cost
                    self.history[(npos, ndir)].add((position, direction))
                    if self.pause:
                        self.visited = [k for (k, v) in dists.keys()]
                    heappush(queue, (dist_cost, (npos, ndir)))

    def show_grid(self) -> str:
        output = ""
        for row_index in range(self.max_x + 1):
            for col_index in range(self.max_y + 1):
                val = self.grid[(col_index, row_index)]
                if (col_index, row_index) in self.visited:
                    output += "X"
                else:
                    output += val
            output += "\n"
        time.sleep(self.pause)
        return output

    def get_solution_path(self, entry) -> set:
        print("Solution", self.history[entry])
        return 'x'
        if entry[0] != self.start:
            hist = self.history[entry]
            return hist.union(
                *[self.get_solution_path(pos) for pos in self.history[entry]]
            )
        return set()
